...
public class RcontentProvider extends ContentProvider
{
    //base objects
    ...
    private RdbHelper Dbinstance;
    private static UriMatcher getUriMatcher()
    {
        UriMatcher uriMatcher = new UriMatcher(UriMatcher.NO_MATCH);//base case
        uriMatcher.addURI(Rcontract.ProviderName, Rcontract.const_path, Data);
        uriMatcher.addURI(Rcontract.ProviderName, Rcontract.const_path + "/#", DataWithId);
        return uriMatcher;
    }
    ...
    public boolean onCreate()
    {
        ...
        Dbinstance= new RdbHelper(context);
        return true;
    }
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder)
    {
        ...
        switch (match) {
            case Data:
                retCursor = db.query(Rcontract.TABLE_NAME,null,null,null,null,null,sortOrder);
                break;
            default:
    ...
    <type> should be unique to the corresponding URI. So now let’s update our code to implement getType as follows:*/ {
        ...
        switch (match) {
            case Data:
                return getContext().getString(R.string.RESPONSE1)+ "/" + Rcontract.ProviderName + "/" + Rcontract.const_path;
            case DataWithId:
                return getContext().getString(R.string.RESPONSE1) + "/" + Rcontract.ProviderName + "/" + Rcontract.const_path;
            default:
                throw new UnsupportedOperationException(getContext().getString(R.string.RESPONSE2) + uri);
    ...
    public Uri insert(Uri uri, ContentValues contentValues)
    {
        ...
        switch (match) {
            case Data:
                long id = db.insertOrThrow(Rcontract.TABLE_NAME,null,contentValues);
                if (id > 0) {
                    returnUri = ContentUris.withAppendedId(Rcontract.CONTENT_URI, id);
                } else {
    ...
    * @param uri The full URI to query, including a row ID (if a specific record is requested).
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs)
    {
        ...
        {
            case Data:
                res=db.delete(Rcontract.TABLE_NAME," TAG = ?",new String[] {selection});
                break;
            default:
