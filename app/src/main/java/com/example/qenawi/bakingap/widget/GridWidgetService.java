package com.example.qenawi.bakingap.widget;
...
class GridRemoteViewsFactory implements RemoteViewsService.RemoteViewsFactory {
    ...
    {
        data.clear();
        Cursor result=mContext.getContentResolver().query(Rcontract.CONTENT_URI,null,null,null,Rcontract.B_id);
        if (result!=null)
        {
            result.moveToFirst();
        }
        ...
        {
            data.add(result.getString(result.getColumnIndex(Rcontract.INGREDIENT_NAME)));
            result.moveToNext();
        }
    }
}
